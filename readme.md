# 🚀 E-Commerce Performance Engineering: End-to-End Observability

![JMeter](https://img.shields.io/badge/Apache%20JMeter-5.5-D22128?logo=apachejmeter&logoColor=white)
![k6](https://img.shields.io/badge/k6-0.54.0-7D64FF?logo=k6&logoColor=white)
![Playwright](https://img.shields.io/badge/Playwright-1.57.0-45ba4b?logo=playwright&logoColor=white)
![Grafana](https://img.shields.io/badge/Grafana-10.0-F46800?logo=grafana&logoColor=white)
![InfluxDB](https://img.shields.io/badge/InfluxDB-1.8-22ADF6?logo=influxdb&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-Compose-2496ED?logo=docker&logoColor=white)


## 🎯 Overview

This repository hosts a robust **Performance Engineering Suite** designed to simulate high-load user journeys on a .NET-based e-commerce platform ([Tricentis Demo Web Shop](https://demowebshop.tricentis.com/)).

Going beyond simple script execution, this project implements a full **Infrastructure as Code (IaC)** observability pipeline. It automatically provisions databases, data sources, and real-time dashboards to monitor both **Backend Health** (Protocol) and **Frontend Experience** (Visual).

![UI Jmeter](https://i.imgur.com/LrWZzwMl.png)


## 🏗️ Architecture & Features

The solution follows a modern "Performance as Code" approach:

* **Hybrid Load Generation (Backend):**
    * **Legacy/Standard:** Apache JMeter for protocol-heavy, thread-based simulation.
    * **Modern/DevOps:** k6 (TypeScript) for lightweight, developer-friendly load testing.
* **Frontend Performance (User Experience):**
    * **Playwright:** Simulates a real browser to measure **Web Vitals** (LCP, FCP) and DOM Interactive time during critical flows (Checkout).
* **Real-Time Observability:**
    * **InfluxDB:** Time-series database storing metrics from JMeter, k6, and Playwright.
    * **Grafana:** Pre-provisioned dashboards (Infrastructure as Code) for immediate analysis.
* **Engineering Logic:**
    * **Dynamic Correlation:** Handling Anti-CSRF tokens and Session Cookies automatically.
    * **Resilient Checkout:** Robust handling of "One Page Checkout" race conditions and AJAX loading states.
    * **Automated Provisioning:** Docker volumes and initialization scripts ensure the environment is "Plug & Play".


## 📂 Project Structure

```text
demo-shop-performace-test/
├── docker-compose.yml              # 🐳 Orchestration (The brain)
├── tests/
│   ├── grafana/
│   │   ├── provisioning/           # ⚙️ Auto-configures Data Sources
│   │   └── dashboards/             # 📊 Dashboards (JMeter, k6 & Playwright)
│   ├── influxdb-init/              # 🗄️ Auto-creates databases on startup
│   ├── jmeter/
│   │   └── Demo-Shop-Performance-Test.jmx         # JMeter Plan
│   ├── k6/
│   │   ├── src/                    # TS Source Code
│   │   └── dist/                   # Compiled JS (Generated by Webpack)
│   └── e2e/
│       ├── specs/                  # Playwright TypeScript Specs
│       └── playwright.config.ts    # Browser & Reporting Config
└── results/                        # Local artifacts (ignored by git)
```


## 🚀 Quick Start

No local installation of Java, Go, or Node.js is required. Docker Compose handles the entire stack.

### ⚡ Option 1: The "One-Shot" Suite (Recommended)

To see the full power of the suite, run the `test:full` script. It executes the shell automation that resets the environment, provisions infrastructure, and runs **JMeter, k6, and Playwright** sequentially.

```bash
# Runs ./run-all.sh via NPM
npm run test:full
````


### 🛠️ Option 2: Manual Step-by-Step

If you prefer to run specific tests or debug individual components, follow these steps:

#### 1️⃣ Start the Infrastructure

Start the Database and Dashboard services first.
*(Wait \~10 seconds for Grafana to provision the dashboards)*.

```bash
docker compose up -d influxdb grafana
```

#### 2️⃣ Run the Tests (Choose your weapon)
Please run one at a time.

**Option A: Run JMeter (Classic Load Test)**
Executes the JMeter plan in non-GUI mode with auto-cleanup.

```bash
docker compose up jmeter
```

**Option B: Run k6 (Modern Load Test)**
Executes the TypeScript-based script compiled via Webpack.

```bash
docker compose up k6
```

**Option C: Run Playwright (Frontend Performance)**
Executes the End-to-End flow in a headless browser container, capturing visual metrics.

```bash
docker compose run --rm playwright
```

#### 3️⃣ Analyze Results (Real-Time)

Open your browser to **[http://localhost:3000](https://www.google.com/search?q=http://localhost:3000)**.

  * **No login required.**
  * **Dashboards are pre-loaded.**
  * Navigate to **Dashboards** folder to see live metrics for JMeter, k6, or Playwright.

-----

## 🧹 Resetting the Environment

Since InfluxDB retains data in Docker volumes, you might want to clean up old test data to start fresh.

```bash
# Stops containers and REMOVES volumes (Database reset)
docker compose down -v
```


-----

## 🧠 Technical Details

### JMeter Implementation

  * **Backend Listener:** Configured to stream metrics asynchronously to InfluxDB.
  * **Auto-Cleanup:** The Docker entrypoint automatically cleans old `.jtl` and HTML reports to prevent data pollution between runs.

![Grafana Jmeter](https://i.imgur.com/9vqB53Fl.png)

### k6 Implementation (TypeScript)

The k6 implementation mimics a modern software project structure:

  * **Webpack:** Bundles imports and modules (like `utils/extraction.ts`) into a single file k6 can understand.
  * **Extraction Logic:** Uses regex and HTML parsing to handle dynamic IDs and CSRF tokens resiliently.
  * **Config:** `webpack.config.js` ensures compatibility between Node modules and the k6 runtime.

![Grafana k6](https://i.imgur.com/EP6NYiTm.png)
### Playwright Implementation (Frontend)

  * **Visual Metrics:** Custom helper functions utilize the **Navigation Timing API** to extract `domComplete` and `first-contentful-paint` (FCP) directly from the browser context.
  * **Resilience:** Advanced handling of AJAX race conditions in the "One Page Checkout" (waiting for accordion animations and loading overlays).
  * **Docker Integration:** Capable of sending metrics from the containerized browser to InfluxDB via internal Docker networking.

![Grafana Playwright](https://i.imgur.com/FRva5BKm.png)

-----
*Author: Luis Fernando Richter*
